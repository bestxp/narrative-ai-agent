...
func TestMarkdownToHTML_Bold(t *testing.T) {
	t.Parallel()
	assert.Equal(t, "<b>hello</b>", telegram.MarkdownToHTML("**hello**"))
}

func TestMarkdownToHTML_Italic(t *testing.T) {
	t.Parallel()
	assert.Equal(t, "<i>hello</i>", telegram.MarkdownToHTML("*hello*"))
}

func TestMarkdownToHTML_Code(t *testing.T) {
	t.Parallel()

	out := telegram.MarkdownToHTML("x`a <b>`y")
...
func TestMarkdownToHTML_Link(t *testing.T) {
	...
	out := telegram.MarkdownToHTML("start [label](https://e.com) end")
...
func TestMarkdownToHTML_StripsLiteralHTML(t *testing.T) {
	...
	out := telegram.MarkdownToHTML("a <b> c & d")
...
func TestMarkdownToHTML_PlainTextIsPassthrough(t *testing.T) {
	t.Parallel()
	assert.Equal(t, "hello world", telegram.MarkdownToHTML("hello world"))
}

func TestMarkdownToHTML_Empty(t *testing.T) {
	t.Parallel()
	assert.Empty(t, telegram.MarkdownToHTML(""))
}

func TestMarkdownToHTML_Russian(t *testing.T) {
	t.Parallel()

	out := telegram.MarkdownToHTML("**Саске** сказал *привет* и `ушел`.")
...
func TestMarkdownToHTML_LiteralAsterisksArithmetic(t *testing.T) {
	...
	out := telegram.MarkdownToHTML("2 * 3 = 6")
...
func TestMarkdownToHTML_Strikethrough(t *testing.T) {
	...
	out := telegram.MarkdownToHTML("~~old~~ stays")
...
func TestMarkdownToHTML_DoubleUnderscoreIsBold(t *testing.T) {
	...
	out := telegram.MarkdownToHTML("__new__ stays")
...
func TestMarkdownToHTML_FencedCodeWithLanguage(t *testing.T) {
	...
	out := telegram.MarkdownToHTML("```go\nfunc x(){}\n```")
...
func TestMarkdownToHTML_Blockquote(t *testing.T) {
	...
	out := telegram.MarkdownToHTML("> quoted line")
...
func TestIsMessageNotModified(t *testing.T) {
	...
	longErr := "Bad Request: message is not modified: specified new message content " +
		"and reply markup are exactly the same as a current content and reply markup of the message"
	assert.True(t, telegram.IsMessageNotModified(errString(longErr)))
	assert.False(t, telegram.IsMessageNotModified(errString("network timeout")))
	assert.False(t, telegram.IsMessageNotModified(nil))
}
...
func TestIsMessageTooLong(t *testing.T) {
	t.Parallel()
	assert.True(t, telegram.IsMessageTooLong(errString("Bad Request: MESSAGE_TOO_LONG")))
	assert.True(t, telegram.IsMessageTooLong(errString("Bad Request: message is too long")))
	assert.True(t, telegram.IsMessageTooLong(errString("400 MESSAGE_TOO_LONG: chat is rate-limited")))
	assert.False(t, telegram.IsMessageTooLong(errString("Bad Request: chat not found")))
	assert.False(t, telegram.IsMessageTooLong(nil))
}
...
func TestSplitForTelegram_ShortText(t *testing.T) {
	...
	out := telegram.SplitForTelegram("hello world")
...
func TestSplitForTelegram_AtParagraphBoundary(t *testing.T) {
	...
	text := para1 + "\n\n" + para2
	out := telegram.SplitForTelegram(text)
	require.Len(t, out, 2, "should split at the \\n\\n boundary, not later")
	assert.Equal(t, para1+"\n\n", out[0])
	assert.Equal(t, para2, out[1])

	for i, c := range out {
		assert.LessOrEqual(t, len(c), telegram.MaxTelegramMessageLen, "chunk %d exceeds cap", i)
	}
}
...
func TestSplitForTelegram_HardCutOnGiantParagraph(t *testing.T) {
	...
	big := strings.Repeat("x", telegram.MaxTelegramMessageLen+500)
	out := telegram.SplitForTelegram(big)
...
func TestSplitForTelegram_RussianAtCut(t *testing.T) {
	...
	// where the cut point at telegram.MaxTelegramMessageLen
	...
	for b.Len() < telegram.MaxTelegramMessageLen*2+100 {
		b.WriteString("Кагуя и Хината смотрят на Саске. ")
	}

	in := b.String()
	out := telegram.SplitForTelegram(in)
	require.GreaterOrEqual(t, len(out), 2)
	...
	for i, c := range out {
		...
		assert.LessOrEqual(t, len([]rune(c)), telegram.MaxTelegramMessageLen,

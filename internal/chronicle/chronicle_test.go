...
func TestCompressWindow_RejectsOverlap(t *testing.T) {
	...
	c.Periods = append(c.Periods, Period{From: 1, To: 30, Memory: "old"})
	...
	// Chronicle should be left untouched.
	if len(c.Periods) != 1 {
...
func TestWindowSize_MatchesLimits(t *testing.T) {
	...
	if WindowSize != 30 {
		t.Errorf("WindowSize = %d, want 30 (limits.MemoriseWindowDays)", WindowSize)
	}
}

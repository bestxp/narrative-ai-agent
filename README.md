...
| `llm` | `providers.<name>.driver` | `"openai"` | `openai` или `anthropic`; per-provider, не per-role |
| | `providers.<name>.api_url` | `"http://localhost:11434/v1"` | OpenAI-compatible chat completions endpoint |
| | `providers.<name>.api_key` | `"ollama"` | Bearer token (Ollama игнорирует, OpenRouter требует настоящий) |
| | `providers.<name>.context_window` | `32000` | soft cap на input-токены; триггерит compaction |
| | `roles.<name>.provider` | — | имя provider'а; обязательно для каждой роли |
| | `token_tracking` | `"estimate"` | `off` / `estimate` / `usage` |
| | `roles.narrative` | — | обязательная роль (model, system_prompt_path) |
| | `roles.summary` | — | роль сжатия (опционально) |
| `slowlog` | `enabled` | `false` | JSON-lines аудит в slow.log |

## LLM providers vs roles

`providers` — это пулы LLM-эндпоинтов (driver + url + key + context_window). `roles` — это способы использования провайдера (narrative, summary, classification); одна роль ссылается на один provider по имени и задаёт только model + параметры per-role. Несколько ролей могут делить один provider (например, narrative и summary на одном Ollama), а одна роль может иметь `model` override поверх дефолта провайдера. Несколько провайдеров полезно, когда нужно смешивать endpoints (локальный Ollama + облачный OpenRouter) без дублирования url/key.

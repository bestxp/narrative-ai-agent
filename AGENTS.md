...
- ** запрет ** `git reset --hard` ни при каких условиях
- ** ALPHA ** нет поддержки обратной совместимости, не делаем fallback, 
не делаем backup игровых, для восстановления есть git, не делаем инструментов миграций, если находишь удаляешь из кода
...
- **Никаких магических чисел в шаблонах `.md.tmpl`** —
  все числовые лимиты (размер выжимки в словах,
  целевые размеры файлов, количество предложений на
  окно, лимиты токенов, границы window) берутся из
  `config.yaml` через data-bag (`{{ .Compaction.* }}`,
  `{{ .Narrative.* }}`), не пишутся литералами вроде
  `150-300 слов` или `200-400 токенов`. Цель
  шаблонизатора — вынести настраиваемые значения из
  текста в конфиг. Числа, не имеющие конфиг-кноба
  (примеры в `## Пример` секциях, порядковые номера
  дней в демо-данных) — допустимы; все, что описывает
  **лимиты/цели/пороги** — должны идти через `{{ .*.Field }}`.
  Grep-команда для проверки:
  ```bash
  grep -nE '[0-9]{3,} слов|[0-9]+-[0-9]+ слов|[0-9]{3,} токен|[0-9]+-[0-9]+ токен|[0-9]{3,} строк|[0-9]+-[0-9]+ строк|[0-9]{2,} предл|[0-9]+-[0-9]+ предл' \
      internal/prompts/*.md.tmpl
  ```
  При срабатывании — добавить поле в `CompactionData`
  / `NarrativeData` (если ещё нет), пробросить дефолт из
  `internal/limits`, заменить литерал на `{{ .Compaction.X }}`.
